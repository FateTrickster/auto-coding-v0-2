...
class TestValidRender:
    def test_real_yaml_renders(self):
        r = render(str(_fixture_yaml()), Path(tempfile.mkdtemp()) / "out")
    ...
    def test_no_placeholders(self):
        p = Path(render(str(_fixture_yaml()), Path(tempfile.mkdtemp()) / "out")["prompt_path"]).read_text(encoding="utf-8")
        assert "缺失" not in p and "未提供" not in p

    def test_arrows_preserved(self):
        p = Path(render(str(_fixture_yaml()), Path(tempfile.mkdtemp()) / "out")["prompt_path"]).read_text(encoding="utf-8")
        assert "primary_code" in p
